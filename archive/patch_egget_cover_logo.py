# -*- coding: utf-8 -*-
"""Bruk logo som linje 2 paa forsiden i det-forsvunne-dinosauregget-scriptene.

Samme moenster som patch_motet_cover_logo.py. To ting skjer:

1. Linje 2 blir et logo-bilde fra script/logo/<locale>/ naar det finnes en logo
   for spraaket. nn bruker den norske logoen med vilje - tittelen er identisk
   paa bokmaal og nynorsk ("Det forsvunne dinosauregget").
2. Tekst-varianten (som fortsatt brukes av en-US/en-GB/sv, der det ikke finnes
   logo enda) krymper naa fonten til tittelen faktisk faar plass. Uten dette
   rant "Det forsvunne dinosauregget" ut over begge kantene paa forsiden -
   fontstoerrelsen var laast til 0.095 av kortsiden uansett tittellengde.

Parametrene under er hentet fra config/next_book_titles.json (nb), altsaa det
brukeren tunet i DP Title Tester.

  python patch_egget_cover_logo.py --dry-run
  python patch_egget_cover_logo.py --apply
"""
...
FILES = [
    "C:/ComfyUI/script/nb/dinosauregget-text-nb.py",
    "C:/ComfyUI/script/nn/dinosauregget-text-nn.py",
    "C:/ComfyUI/script/en-US/dinosauregget-text-en-US.py",
    "C:/ComfyUI/script/en-GB/dinosauregget-text-en-GB.py",
    "C:/ComfyUI/script/sv/dinosauregget-text-sv.py",
]
...
# --- krymp-til-passe i tekst-varianten -------------------------------------
# Filene bygger fonten paa to ulike maater (andel av kortsiden i nb/nn/en-GB,
# absolutte px * cover_scale i en-US/sv), saa vi erstatter dem ikke - vi
# skyter inn en krympe-runde rett etter. line1/line2/draw/w finnes allerede.
FIT_RE = re.compile(
    r"^(?P<ind>[ \t]*)font_small = ImageFont\.truetype\(COVER_FONT,.*\)\n"
    r"(?P=ind)font_large = ImageFont\.truetype\(COVER_FONT,.*\)\n",
    re.M,
)
...
def patch(src: str) -> str:
    ...
    if not FIT_RE.search(src):
        raise SystemExit("fant ikke font-oppsettet i tekst-varianten")

    src = src.replace(OLD_SIG, NEW_SIG, 1)
    src = FIT_RE.sub(lambda m: m.group(0) + FIT_INSERT, src, count=1)
