...
ENHJORNING_DREAMPAGE_FIRST = os.path.join(
    os.path.dirname(SCRIPT_ROOT_DIR), "books", "enhjorning", "dreampage-first-rosa.png"
)
DREAMPAGE_FIRST_TAGLINE = "Trykt med omtanke\nfor kvalitet"
...
COVER_LINE_SPACING = 0.046
COVER_SMALL_SIZE = 378
COVER_LARGE_SIZE = 405
...
def remove_white_background(img: Image.Image, thresh: int = 25) -> Image.Image:
    ...
    # Logoer som allerede har alpha er ferdig utklippet fra for. Kjorer vi
    # hvitfjerning pa dem, blir selve bokstavene gjennomsiktige (de er hvite
    # med farget kontur), og tittelen ser ut som den forsvinner inn i
    # illustrasjonen. Bare logoer uten alpha trenger denne behandlingen.
...
def draw_centered_title_cover(img: Image.Image, text: str) -> None:
    ...
    # Skyggen ma ha luft rundt seg. Blurres alfaen i et lerret pa noyaktig
    # logostorrelse, blir gradienten kuttet ved kanten og legger igjen en
    # synlig gra firkant bak tittelen.
    blur_radius = max(8, int(target_h * 0.20))
    pad = blur_radius * 3
    padded_alpha = Image.new("L", (logo.width + pad * 2, logo.height + pad * 2), 0)
    padded_alpha.paste(logo.getchannel("A"), (pad, pad))
    shadow_alpha = padded_alpha.filter(ImageFilter.GaussianBlur(blur_radius))
    shadow_layer = Image.new("RGBA", padded_alpha.size, (*COVER_LOGO_SHADOW, 180))
    shadow_layer.putalpha(shadow_alpha)
    shadow.alpha_composite(shadow_layer, (x2 - pad + max(2, int(target_h * 0.04)),
                                          y2 - pad + max(2, int(target_h * 0.05))))
    ...
    story: List[Dict[str, Any]] = [
        {
            ...
            "side": "left",
            "block": story_block(
                "Ein heilt vanleg ettermiddag gjekk (navn) i skogen.\n"
                "Midt i den mjuke jorda låg noko rart.\n"
                "Små hovspor, tett etter kvarandre.\n"
                "Og kvart einaste spor glitra svakt som sølv."
                ,
                child_name,
                ["hovspor", "glitra", "sølv", "rart"],
                side="left",
            ),
        },
        {
            ...
            "side": "right",
            "block": story_block(
                "(navn) følgde spora djupare inn i skogen.\n"
                "Små lys sveva mellom trea, og ukjende blomar opna seg.\n"
                "Til slutt stoppa hovspora framfor ein høg foss.\n"
                "Bak vatnet lyste noko svakt og gyllent."
                ,
                child_name,
                ["spora", "lys", "blomar", "foss", "gyllent"],
                side="right",
            ),
        },
        # Side 3 - venstre kvadrat (statisk handbilde, baerer teksten)
        {
            ...
            "block": story_block(
                "(navn) strekte handa forsiktig inn i fossen.\n"
                "Vatnet var lunka, og det glitra rundt fingrane.\n"
                "Bak fossen låg ein smal tunnel i fjellet.\n"
                "(navn) gjekk gjennom, og heile dalen opna seg."
                ,
                child_name,
                ["handa", "fossen", "glitra", "tunnel", "dalen"],
                side="left",
            ),
        },
        ...
        },
        {
            ...
            "side": "right",
            "block": story_block(
                "Ein liten einhyrning kom springande mellom blomane.\n"
                "Han stoppa rett framfor (navn) og såg på henne.\n"
                "Men hornet lyste berre svakt.\n"
                "Så riste bakken, og eit djupt drønn kom frå fjella."
                ,
                child_name,
                ["einhyrning", "hornet", "svakt", "drønn"],
                side="right",
            ),
        },
        {
            ...
            "side": "left",
            "block": story_block(
                "Einhyrningen sprang mot ein stor, klar innsjø.\n"
                "Over vatnet sveva tre lysande stjernesteinar.\n"
                "Ein blå, ein grøn og ein lilla.\n"
                "Men plassen der den fjerde skulle vore, var mørk."
                ,
                child_name,
                ["innsjø", "stjernesteinar", "blå", "grøn", "lilla", "mørk"],
                side="left",
            ),
        },
        {
            ...
            "side": "right",
            "block": story_block(
                "Den eldste einhyrningen kom fram til vasskanten.\n"
                "Hornet skein, og eit bilete dukka opp i innsjøen.\n"
                "(navn) såg ein mørk skapnad snike seg mellom trea.\n"
                "Den fjerde stjernesteinen var stolen."
                ,
                child_name,
                ["eldste", "Hornet", "bilete", "skapnad", "stolen"],
                side="right",
            ),
        },
        {
            ...
            "side": "left",
            "block": story_block(
                "Utan steinen ville magien i dalen forsvinne.\n"
                "Den vesle einhyrningen stampa bestemt i bakken.\n"
                "Saman følgde dei eit spor av svarte, glitrande merke.\n"
                "Sporet førte dei langt bort frå innsjøen."
                ,
                child_name,
                ["magien", "bestemt", "spor", "merke"],
                side="left",
            ),
        },
        {
            ...
            "side": "right",
            "block": story_block(
                "Stien førte dei inn i Krystallskogen.\n"
                "Enorme gjennomsiktige tre kasta regnbogar over bakken.\n"
                "Brått byrja greinene å røre seg.\n"
                "Stien bak dei forsvann, og dei var fanga."
                ,
                child_name,
                ["Krystallskogen", "regnbogar", "greinene", "fanga"],
                side="right",
            ),
        },
        {
            ...
            "side": "left",
            "block": story_block(
                "(navn) fann dei mørke spora igjen på bakken.\n"
                "Dei følgde dei heilt ut på den andre sida.\n"
                "Framfor dei låg ei djup kløft.\n"
                "På kanten stod eit lite hus som lyste i mørket."
                ,
                child_name,
                ["spora", "kløft", "lite hus", "lyste"],
                side="left",
            ),
        },
        {
            ...
            "side": "right",
            "block": story_block(
                "Ei gammal steinbru strekte seg over kløfta.\n"
                "Dei gjekk forsiktig ut på dei gamle steinane.\n"
                "Midt på brua kom eit nytt drønn, og steinane sprakk.\n"
                "(navn) greidde akkurat å kome seg over."
                ,
                child_name,
                ["steinbru", "kløfta", "drønn", "sprakk"],
                side="right",
            ),
        },
        {
            ...
            "side": "left",
            "block": story_block(
                "Inne i huset låg stjernesteinen på ei søyle.\n"
                "(navn) lyfte han forsiktig opp.\n"
                "Med eitt lyste heile rommet.\n"
                "På veggen kom eit stort symbol til syne: ein svart halvmåne."
                ,
                child_name,
                ["stjernesteinen", "søyle", "lyste", "halvmåne"],
                side="left",
            ),
        },
        {
            ...
            "side": "right",
            "block": story_block(
                "Dei skunda seg tilbake til innsjøen.\n"
                "Då steinen kom på plass, skaut fire lysstrålar mot himmelen.\n"
                "Magien strøymde tilbake gjennom heile dalen.\n"
                "Blomane vakna, og elvane glitra igjen."
                ,
                child_name,
                ["lysstrålar", "Magien", "vakna", "glitra"],
                side="right",
            ),
        },
        {
            ...
            "side": "left",
            "block": story_block(
                "Heile flokken samla seg rundt (navn).\n"
                "Den eldste einhyrningen bøygde hovudet mot henne.\n"
                "Eit lite sølvmerke byrja å gløde på handa hennar.\n"
                "(navn) hadde blitt ein ven av Einhyrningdalen."
                ,
                child_name,
                ["flokken", "eldste", "sølvmerke", "ven"],
                side="left",
            ),
        },
        {
            ...
            "side": "right",
            "block": story_block(
                "Om kvelden var (navn) heime igjen.\n"
                "Men utanfor huset låg eit svart, glitrande spor.\n"
                "Ved sida av låg ein stein med ein svart halvmåne.\n"
                "Og langt borte lyste noko lilla mellom fjella."
                ,
                child_name,
                ["heime", "spor", "halvmåne", "lilla"],
                side="right",
            ),
        },
    ]
            ...
            "blocks": [
                {
                    "text": pformat(
                        ...
                        "Må historien minne deg på at hjertet ditt vet veien.",
                        child_name,
                    ),
                    "font_size": 116,
                    "color": "#17121F",
                    "highlights": [child_name, "helten", "farger", "mot", "magi", "hjertet"],
                    # Avstanden regnes na ut fra tittelens bunn i render_blank_page.
                    "y_offset": 0,
                }
            ],
        },
        ...
        [
            {
                ...
                # Bildet hentes fra script/lastpages/<locale>/, men siden ma hete
                # blank-back.png ut - dream_pdf_guard dropper alt som heter Lastpage*.
                "source": "Lastpage(enhjørning).png",
                "type": "inner",
                "side": "left",
                "blank_only": True,
            },
            {
                ...
                "blocks": [
                    {
                        "text": pformat(
                            "Då (navn) fann glitrande hovspor i skogen, førte dei til ein dal ingen kjende til.\n"
                            "I Einhyrningdalen er ein av dei fire stjernesteinane stolen, og magien held på å slokne.\n"
                            "Saman med ein liten einhyrning følgjer (navn) eit mørkt spor gjennom Krystallskogen, over ei sprukken steinbru og inn i eit lite hus i fjellet.\n"
                            "Ei personleg forteljing om mot, vennskap og om å tore å følgje sporet ein ser.\n"
                            "Nokre eventyr er ekte likevel.",
                            child_name,
                        ),
                        "font_size": 82,
                        "color": "#FFFFFF",
...
def render_blank_page(page: Dict[str, Any], out_dir: str) -> List[str]:
    blank_path = ENHJORNING_DREAMPAGE_FIRST if os.path.exists(ENHJORNING_DREAMPAGE_FIRST) else os.path.join(SCRIPT_DIR, "dreampage-first.png")
    ...
    # Tittelen starter hoyere oppe, og linjene far luft som folger
    # skriftstorrelsen i stedet for en fast 16 px (som forsvinner helt pa
    # et 4096 px lerret).
    y = int(height * 0.105)
    title_line_gap = int(title_font.size * 0.16)
    for line in title_lines:
        tw, th = text_size(draw, line, title_font)
        draw.text(((width - tw) // 2, y), line, font=title_font, fill="#17121F")
        y += th + title_line_gap
    title_bottom = y - title_line_gap

    for block in page.get("blocks", []):
        font = ImageFont.truetype(INNER_FONT, block.get("font_size", 38))
        # Brodteksten henger under tittelen i stedet for a sitte pa en fast
        # hoyde, og spalten er smalere sa linjene far en roligere lengde.
        box = (
            int(width * 0.09),
            title_bottom + int(height * 0.10) + int(block.get("y_offset", 0)),
            int(width * 0.91),
            int(height * 0.88),
        )
        draw_text(
            ...
            line_spacing=max(18, int(font.size * 0.55)),
    ...
    _dpf_tag_gap = int(_dpf_tag_fs * 0.55)
    ...
    _dpf_ty = int(height * 0.715) - _dpf_tag_total // 2
...
def render_page(page: Dict[str, Any], base_dir: str, out_dir: str) -> List[str]:
    if page.get("blank_only"):
        ...
        # "source" lar siden hete blank-back.png ut, men hente bildet fra
        # lastpages-mappa - ellers hadde dream_pdf_guard droppet den.
        ...
        if source is None:
            source = resolve_final_inner_path(page.get("source") or page["filename"], base_dir)
        if source is None:
            return []
    ...
    if page.get("type") == "cover":
        if page["filename"].lower().startswith("forside"):
            ...
            for block in page.get("blocks", []):
                ...
                # Samme myke skygge bak hele bolken som motet-i-hjertet bruker,
                # ellers drukner den hvite teksten i den lyse illustrasjonen.
                if block.get("text_backdrop", True):
                    draw_text_backdrop(
                        img, (x1, y1, x2, y2), block["text"], font,
                        max(6, int(16 * scale_y)),
                        highlights=block.get("highlights", []),
            ...
            try:
                ...
                ly = img_h - logo.height - int(img_h * 0.06)
                # Logoen er ren hvit tekst. Uten skygge forsvinner den i lyse
                # bakside-illustrasjoner og ser gjennomsiktig ut - samme
                # dropshadow som forsidelogoen far gjor den lesbar overalt.
                blur_radius = max(6, int(logo.height * 0.18))
                pad = blur_radius * 3
                padded_alpha = Image.new("L", (logo.width + pad * 2, logo.height + pad * 2), 0)
                padded_alpha.paste(logo.getchannel("A"), (pad, pad))
                shadow_alpha = padded_alpha.filter(ImageFilter.GaussianBlur(blur_radius))
                shadow_layer = Image.new("RGBA", padded_alpha.size, (*COVER_LOGO_SHADOW, 190))
                shadow_layer.putalpha(shadow_alpha)
                img.alpha_composite(shadow_layer,
                                    (lx - pad + max(2, int(logo.height * 0.04)),
                                     ly - pad + max(2, int(logo.height * 0.05))))
...
_DREAMPAGE_TRANSLATIONS = {
  "{name} og\nEnhjørningsdalen": "{name} og\nEinhyrningdalen",
  "Denne boken er laget spesielt for deg, {name}.\nDu er helten i et eventyr fylt med farger, mot og magi.\nMå historien minne deg på at hjertet ditt vet veien.": "Denne boka er laga spesielt for deg, {name}.\nDu er helten i eit eventyr fylt med fargar, mot og magi.\nMå historia minne deg på at hjartet ditt veit vegen.",
  "Det var en gang en jente som het {name}.\nHun elsket farger, fantasi og historier om skjulte verdener.\nOfte føltes det som om noe ventet på henne.\nSom om eventyret bare manglet det riktige øyeblikket.": "Det var ein gong ei jente som heitte {name}.\nHo elska fargar, fantasi og historier om skjulte verder.\nOfte kjendest det som om noko venta på henne.\nSom om eventyret berre mangla den rette augneblinken.",
  "En dag oppdaget {name} noe uvanlig.\nEt svakt spor av regnbuefarger lyste foran henne.\nDet var nesten usynlig, men hun kunne se det.\nOg det føltes som om sporet kalte på henne.": "Ein dag oppdaga {name} noko uvanleg.\nEit svakt spor av regnbogefargar lyste framfor henne.\nDet var nesten usynleg, men ho kunne sjå det.\nOg det kjendest som om sporet kalla på henne.",
  "{name} stoppet opp og tenkte seg om.\nHun kunne snu og gå tilbake, akkurat som før.\nEller hun kunne følge sporet videre.\nHjertet hennes banket litt raskere.": "{name} stoppa opp og tenkte seg om.\nHo kunne snu og gå tilbake, akkurat som før.\nEller ho kunne følgje sporet vidare.\nHjartet hennar banket litt raskare.",
  "Da {name} tok et steg frem, skjedde det noe.\nLuften rundt henne forandret seg.\nFargene ble sterkere og klarere enn før.\nHun visste at hun hadde funnet noe hemmelig.": "Då {name} tok eit steg fram, skjedde det noko.\nLufta rundt henne forandra seg.\nFargane vart sterkare og klarare enn før.\nHo visste at ho hadde funne noko hemmeleg.",
  "Foran {name} åpnet det seg en dal.\nEn dal full av lys, liv og farger.\nHun hadde aldri sett noe lignende.\nDet var som om verden smilte til henne.": "Framfor {name} opna det seg ein dal.\nEin dal full av lys, liv og fargar.\nHo hadde aldri sett noko liknande.\nDet var som om verda smilte til henne.",
  "Plutselig kjente {name} at hun ikke var alene.\nNoe beveget seg stille i det fjerne.\nHun ble ikke redd, bare nysgjerrig.\nDette stedet føltes trygt og riktig.": "Plutseleg kjende {name} at ho ikkje var åleine.\nNoko bevega seg stille i det fjerne.\nHo vart ikkje redd, berre nysgjerrig.\nDenne staden kjendest trygt og rett.",
  "Da fikk {name} øye på dem.\nEnhjørningene kom rolig frem, én etter én.\nDe så på henne med kloke, vennlige øyne.\nSom om de hadde ventet på akkurat henne.": "Då fekk {name} auge på dei.\nEinhjørningane kom roleg fram, éin etter éin.\nDei såg på henne med kloke, vennlege auge.\nSom om dei hadde venta på akkurat henne.",
  "Enhjørningene fortalte om dalen.\nDette stedet var hemmelig og sårbart.\nBare noen få fikk lov til å se det.\nOg nå hvilte ansvaret hos {name}.": "Einhjørningane fortalde om dalen.\nDenne staden var hemmeleg og sårbart.\nBerre nokre få fekk lov til å sjå det.\nOg no kvilte ansvaret hos {name}.",
  "{name} kjente en uro i magen.\nHva om hun ikke var god nok?\nHva om hun gjorde noe feil?\nHun så rundt seg og tok et dypt pust.": "{name} kjende ei uro i magen.\nKva om ho ikkje var god nok?\nKva om ho gjorde noko feil?\nHo såg rundt seg og tok eit djupt pust.",
  "Da lyttet {name} til hjertet sitt.\nHun visste ikke alt, men hun visste én ting.\nHun ville gjøre det rette.\nOg noen ganger er det nok.": "Då lytta {name} til hjartet sitt.\nHo visste ikkje alt, men ho visste éin ting.\nHo ville gjere det rette.\nOg av og til er det nok.",
  "Plutselig glødet dalen sterkere igjen.\nFargene blomstret rundt {name}.\nLyset vendte tilbake, klarere enn før.\nValget hennes hadde betydd noe.": "Plutseleg glødde dalen sterkare igjen.\nFargane blomstra rundt {name}.\nLyset vende tilbake, klarare enn før.\nValet hennar hadde betydd noko.",
  "Enhjørningene samlet seg rundt {name}.\nDe stolte på henne nå.\nHun følte seg rolig og stolt.\nIkke fordi hun var perfekt, men fordi hun hadde våget.": "Einhjørningane samla seg rundt {name}.\nDei stolte på henne no.\nHo kjende seg roleg og stolt.\nIkkje fordi ho var perfekt, men fordi ho hadde våga.",
  "Det var på tide å dra tilbake.\n{name} visste at hun ikke trengte å være trist.\nNoen farvel betyr ikke slutten.\nDe betyr at noe viktig alltid vil være med deg.": "Det var på tide å dra tilbake.\n{name} visste at ho ikkje trong å vere trist.\nNokre farvel betyr ikkje slutten.\nDei betyr at noko viktig alltid vil vere med deg.",
  "Verden utenfor så nesten lik ut som før.\nMen {name} var ikke den samme.\nHun bar en hemmelighet i hjertet sitt.\nNoen eventyr er ekte, selv når ikke alle kan se dem.": "Verda utanfor såg nesten lik ut som før.\nMen {name} var ikkje den same.\nHo bar ein løyndom i hjartet sitt.\nNokre eventyr er ekte, sjølv når ikkje alle kan sjå dei.",
  "Denne boken handler om å oppdage noe bare noen få kan se.\nOm å følge et spor når hjertet sier at noe er viktig.\nDen handler om stille mot, om å ta valg og om å ta vare på en hemmelighet.\nI denne historien er det {name} som blir valgt, fordi hun tør å lytte til seg selv.\nEn personlig fortelling som gir barn trygghet, selvtillit og troen på at noen eventyr er ekte.": "Denne boka handlar om å oppdage noko berre nokre få kan sjå.\nOm å følgje eit spor når hjartet seier at noko er viktig.\nDen handlar om stille mot, om å ta val og om å ta vare på ein løyndom.\nI denne historia er det {name} som blir vald, fordi ho tør å lytte til seg sjølv.\nEi personleg forteljing som gir barn tryggleik, sjølvtillit og trua på at nokre eventyr er ekte."
}
