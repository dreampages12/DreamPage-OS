...
def render_page(page: Dict[str, Any], base_dir: str, out_dir: str) -> List[str]:
    ...
    # ------------------------ ryggrad (hentes fra script/ryggrad) ------------------------
    # MÅ ligge før base_path-eksistenssjekken under. ryggrad.png ligger normalt
    # IKKE i ordre-input — den hentes fra script/ryggrad/<slug>/. Tidligere bailet
    # den tidlige "Fant ikke bilde"-returen ut før ryggrad ble resolvet, så covers
    # ble bygget uten ryggrad når den manglet i ordre-input.
    if os.path.basename(page.get("filename", "")).lower() == "ryggrad.png":
        ryggrad_src = resolve_ryggrad_path(base_dir)
        if not ryggrad_src:
            print("Fant ingen ryggrad (verken i script/ryggrad eller ordre-input):", page["filename"])
            return []
        r_img = Image.open(ryggrad_src).convert("RGBA")
        r_out = os.path.join(out_dir, page["filename"])
        os.makedirs(out_dir, exist_ok=True)
        r_img.save(r_out)
        print("Lagret ryggrad:", r_out, "(kilde:", ryggrad_src + ")")
        return [r_out]
