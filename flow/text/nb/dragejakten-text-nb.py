...
def resolve_final_inner_path(filename, base_dir):
    ...
    # Bok-spesifikk siste innerside. prepare_order kopierer ALLTID den delte
    # blank-back.png inn i ordren, saa eksistens alene sier ingenting - bare en
    # fil som SKILLER seg fra malen er ordrens egen "Fortsett eventyret"-side
    # (se build_last_page.py). Er den lik malen, bruker vi dragejaktens egen.
    if os.path.basename(filename).lower() == "blank-back.png" and os.path.exists(DRAGE_BLANK_BACK):
        shared = os.path.join(SCRIPT_DIR, "blank-back.png")
        is_continue_page = os.path.exists(source) and not (
            os.path.exists(shared)
            and filecmp.cmp(source, shared, shallow=False)
        )
        if not is_continue_page:
            return DRAGE_BLANK_BACK
        print("[INNER PDF] Fortsett-eventyret-siden erstatter dragejaktens siste side:", source)
        return source
...
# Dragejaktens egen siste innerside ("Dette eventyret er over ..." med drager).
