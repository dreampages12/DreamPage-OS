...
STYRKEN_DREAMPAGE_FIRST = os.path.join(
    os.path.dirname(SCRIPT_ROOT_DIR), "books", "den-skjulte-styrken", "dreampage-first-styrken.png"
)
...
def render_dreampage_first(child_name: str, out_dir: str) -> str:
    ...
    """
    src = STYRKEN_DREAMPAGE_FIRST if os.path.exists(STYRKEN_DREAMPAGE_FIRST) else os.path.join(SCRIPT_DIR, "dreampage-first.png")
    ...
    body_lines = [
        ...
        "",
        "Inuti dig bor en dold styrka,",
        "varm och levande, starkare än du tror.",
        "",
...
def process_book(base_dir: str, out_dir: str, child_name: str, cover_type: str, gelato_api_key: str):
    ...
    dreampage_first = STYRKEN_DREAMPAGE_FIRST if os.path.exists(STYRKEN_DREAMPAGE_FIRST) else os.path.join(SCRIPT_DIR, "dreampage-first.png")
...
_DREAMPAGE_TRANSLATIONS = {
  "{name} og\nDen skjulte styrken": "{name} och\nDen Dolda Kraften",
  "Det var en helt vanlig morgen, av den typen der sola kiler i øynene og lufta lukter ny dag. {name} gikk den samme veien til skolen som alltid, men i dag føltes alt litt ekstra spennende.\nSekken dunket lett mot ryggen, og inni magen kriblet det av forventning. For i dag skulle hele klassen på tur, og det hadde {name} gledet seg til i ukevis.": "Det var en helt vanlig morgon, en sådan där solen kittlar i ögonen och luften doftar ny dag. {name} gick samma väg till skolan som alltid, men i dag kändes allt lite extra spännande.\nRyggsäcken dunkade lätt mot ryggen, och i magen pirrade det av förväntan. För i dag skulle hela klassen på utflykt, och det hade {name} sett fram emot i flera veckor.",
  "Plutselig stoppet {name} midt i steget. En merkelig lyd kom fra et smalt smug ved siden av gata, som en svak summing blandet med et fjernt klingende ekko.\n{name} klarte ikke å la være. Han gikk forsiktig nærmere, helt til han kikket inn i mørket. Der, midt i lufta, glødet noe varmt og oransje. Hva i all verden kunne det være?": "Plötsligt stannade {name} mitt i steget. Ett märkligt ljud kom från en smal gränd bredvid gatan, som ett svagt surr blandat med ett avlägset klingande eko.\n{name} kunde inte låta bli. Han gick försiktigt närmare, ända tills han kikade in i mörkret. Där, mitt i luften, glödde något varmt och orange. Vad i hela världen kunde det vara?",
  "Lyset ble så sterkt at {name} måtte myse. Hele smuget badet i en gnistrende glød.\nDa lyset roet seg, sto det noe foran ham. Et skimrende hologram svevde i lufta, og bak det hang en superhelt-drakt.\nHologrammet smilte og sa: «Du er den utvalgte, {name}. Du er byens eneste håp.»": "Ljuset blev så starkt att {name} måste kisa. Hela gränden badade i ett gnistrande sken.\nNär ljuset lugnade sig stod något framför honom. Ett skimrande hologram svävade i luften, och bakom det hängde en superhjältedräkt.\nHologrammet log och sa: \"Du är den utvalde, {name}. Du är stadens enda hopp.\"",
  "{name} løp så fort han kunne bort fra smuget, mens hjertet banket hardt i brystet. Han var redd, helt skikkelig redd.\n«Jeg er ikke klar for dette,» hvisket {name} til seg selv. «Jeg er jo bare en helt vanlig gutt.»\nInni seg kjente han både tårer og en liten klump av sinne. Hvorfor måtte det være akkurat han som var den utvalgte?": "{name} sprang så fort han kunde bort från gränden, medan hjärtat bankade hårt i bröstet. Han var rädd, riktigt rädd.\n\"Jag är inte redo för det här\", viskade {name} för sig själv. \"Jag är ju bara en helt vanlig pojke.\"\nInombords kände han både tårar och en liten klump av ilska. Varför måste det vara just han som var den utvalde?",
  "{name} klatret opp til favorittstedet sitt. Derfra kunne han se utover hele byen.\nHan sto stille og tenkte. Langt der nede så han mørke skygger bevege seg sakte mot sentrum.\nVar det dette hologrammet hadde ment? Skulle han virkelig redde byen alene?": "{name} klättrade upp till sin favoritplats. Därifrån kunde han se ut över hela staden.\nHan stod stilla och tänkte. Långt där nere såg han mörka skuggor röra sig sakta mot centrum.\nVar det detta hologrammet hade menat? Skulle han verkligen rädda staden ensam?",
  "Akkurat da kom superheltdrakten flyvende ut av det blå. Den snurret raskt rundt {name} – én gang, to ganger – og i neste øyeblikk hadde han den på seg.\n{name} tok et dypt sats, og med et høyt Swoooosh fløy han rett mot sentrum og landet midt i gata. {name} hadde blitt en superhelt.": "Just då kom superhjältedräkten flygande från ingenstans. Den snurrade snabbt runt {name}, en gång, två gånger, och i nästa ögonblick hade han den på sig.\n{name} tog sats, och med ett högt Swoooosh flög han rakt mot centrum och landade mitt på gatan. {name} hade blivit en superhjälte.",
  "{name} kjente at hjertet fortsatt banket av redsel, men nå kunne han ikke snu. Han tok et dypt pust og tenkte på alle menneskene som trengte hjelp.\nLangt der nede hadde skyggene sendt en søyle av sterkt, oransje lys rett opp mot himmelen. {name} visste at det var dit han måtte dra.": "{name} kände att hjärtat fortfarande bankade av rädsla, men nu kunde han inte vända om. Han tog ett djupt andetag och tänkte på alla människor som behövde hjälp.\nLångt där nere hade skuggorna skickat en pelare av starkt, orange ljus rakt upp mot himlen. {name} visste att det var dit han måste.",
  "Nå sto {name} rett foran det sterke lyset, så nær at han kjente varmen mot kinnene. Dette var øyeblikket hologrammet hadde snakket om.\n{name} stilte seg støtt på bakken, knyttet nevene hardt og gjorde seg klar til å kjempe.": "Nu stod {name} rakt framför det starka ljuset, så nära att han kände värmen mot kinderna. Det här var ögonblicket hologrammet hade talat om.\n{name} ställde sig stadigt på marken, knöt nävarna hårt och gjorde sig redo att kämpa.",
  "Mørket samlet seg foran {name} og vokste til en høy skygge midt i gata. Den vred seg i luften, som om den var laget av røyk og torden.\n{name} kjente redselen prikke i hendene, men han ble stående. Inni brystet våknet en varm styrke, og han visste at han måtte holde ut litt til.": "Mörkret samlades framför {name} och växte till en hög skugga mitt på gatan. Den vred sig i luften, som om den var gjord av rök och åska.\n{name} kände rädslan sticka i händerna, men han stod kvar. Inne i bröstet vaknade en varm kraft, och han visste att han måste hålla ut lite till.",
  "{name} strakte begge armene rett opp mot lyset og knyttet nevene så hardt han kunne.\nPlutselig sprutet et sterkt, blått lys ut av hendene hans og traff den mørke skyggen med et kraftig BANG som fikk hele byen til å riste.": "{name} sträckte båda armarna rakt upp mot ljuset och knöt nävarna så hårt han kunde.\nPlötsligt sprutade ett starkt, blått ljus ut ur hans händer och träffade den mörka skuggan med ett kraftigt BANG som fick hela staden att skaka.",
  "Takket være {name} var byen trygg igjen. Lyset var borte, skyggene forsvunnet, og menneskene begynte å smile.\n{name} hadde alltid trodd at han bare var en vanlig gutt. Men nå visste han noe viktig: han var en ekte superhelt.": "Tack vare {name} var staden trygg igen. Ljuset var borta, skuggorna hade försvunnit, och människorna började le.\n{name} hade alltid trott att han bara var en vanlig pojke. Men nu visste han något viktigt: han var en riktig superhjälte.",
  "Plutselig dukket hologrammet opp igjen, lysende svakt og vennlig i lufta foran ham.\n«Veldig bra jobbet, {name},» sa hologrammet stolt. «Hvis byen trenger hjelp igjen, finner jeg deg.»\n{name} smilte. «Jeg er alltid klar.»": "Plötsligt dök hologrammet upp igen, svagt lysande och vänligt i luften framför honom.\n\"Mycket bra jobbat, {name}\", sa hologrammet stolt. \"Om staden behöver hjälp igen hittar jag dig.\"\n{name} log. \"Jag är alltid redo.\"",
  "{name} tok et siste blikk utover byen. Alt var stille nå, og lysene blinket fredelig i kveldsmørket.\nHan tenkte på alt som hadde skjedd. {name} smilte, for innerst inne visste han noe viktig: Det var han som var den utvalgte.": "{name} tog en sista blick ut över staden. Allt var stilla nu, och ljusen blinkade fridfullt i kvällsmörkret.\nHan tänkte på allt som hade hänt. {name} log, för innerst inne visste han något viktigt: Det var han som var den utvalde.",
  "{name} hadde byttet tilbake til sine vanlige klær og gikk den samme veien til skolen som alltid.\nKlassen var allerede på tur, og han hadde gått glipp av den. «Litt kjipt,» lo {name}. «Men nå har jeg en helt utrolig historie å fortelle vennene mine.»": "{name} hade bytt tillbaka till sina vanliga kläder och gick samma väg till skolan som alltid.\nKlassen var redan på utflykt, och han hade missat den. \"Lite trist\", skrattade {name}. \"Men nu har jag en helt otrolig historia att berätta för mina vänner.\"",
  "Denne boken handler om mot – om å være redd og likevel våge å prøve. Den viser at helter ikke alltid er store og sterke, men ofte vanlige barn som finner styrken i seg selv når det virkelig gjelder.\nDerfor er det {name} som er helten i denne historien. En personlig fortelling som gir barn trygghet, selvtillit og troen på seg selv.": "Den här boken handlar om mod, om att vara rädd och ändå våga försöka. Den visar att hjältar inte alltid är stora och starka, utan ofta vanliga barn som hittar kraften inom sig när det verkligen gäller.\nDärför är det {name} som är hjälten i den här berättelsen. En personlig berättelse som ger barn trygghet, självförtroende och tron på sig själva."
}
