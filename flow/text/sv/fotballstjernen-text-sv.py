...
def resolve_front_cover_logo(base_dir: str | None = None) -> str | None:
    ...
    candidates = [
        os.path.join(LOGO_DIR, "fotbollsstjarna-logo-sv.png"),
        os.path.join(LOGO_DIR, "fotbollsstjarna-logo.png"),
...
def draw_centered_title_cover(img, text, base_dir: str | None = None):
    ...
    font_small = ImageFont.truetype(COVER_TITLE_FONT, max(50, int(base * (84 / 1024))))
...
def build_pages(child_name: str) -> List[Dict[str, Any]]:
    ...
    pages: List[Dict[str, Any]] = [
        ...
        # Side 4 venstre – statisk bomskudd uten synlig ansikt
        {
        ...
        # Side 4 høyre – personlig, headswappet reaksjonsbilde
        {
...

        {
            ...
            "square": True,
            "endtext": True,
            "text": p("Detta äventyr är över,\nmen magin lever vidare."),
        },
...
FOTBALL_DREAMPAGE_FIRST = os.path.join(
    os.path.dirname(SCRIPT_ROOT_DIR), "books", "fotballstjernen", "dreampage-first(fotballstjernen).png"
)
...
def render_dreampage_first(child_name: str, out_dir: str) -> str:
    ...
    body_lines = [
        ...
        "och målen som fyller dessa sidor.",
        "",
...
_DREAMPAGE_TRANSLATIONS = {
  ...
  "{name} sto på banen med ballen ved foten og kjente gresset kile under skoene.\nHan hadde drømt om å bli en ekte fotballstjerne så lenge han kunne huske.\n\nI dag virket målet litt nærmere enn før. Han trakk pusten dypt og hvisket: «Jeg skal prøve så godt jeg kan.»": "{name} stod på planen med bollen vid foten och kände gräset kittla under skorna.\nHan hade drömt om att bli en riktig fotbollsstjärna så länge han kunde minnas.\n\nI dag kändes målet lite närmare än förut. Han drog ett djupt andetag och viskade: \"Jag ska göra mitt bästa.\"",
  "{name} så opp mot lyset som skinte over banen.\nDet glitret i gresset, og hele stedet føltes som starten på noe viktig.\n\nHan tenkte på alle gangene han hadde sparket ball i hagen, bommet, ledd og prøvd igjen. Kanskje var det nettopp slik store drømmer begynte.": "{name} såg upp mot ljuset som sken över planen.\nDet glittrade i gräset, och hela platsen kändes som början på något viktigt.\n\nHan tänkte på alla gånger han hade sparkat boll i trädgården, missat, skrattat och försökt igen. Kanske var det just så stora drömmar började.",
  "{name} begynte å øve med ballen, først sakte og så litt raskere.\nHan prøvde å drible slik de store spillerne gjorde på TV, men ballen trillet ikke alltid dit han ville.\n\nHan bet tennene forsiktig sammen. «Jeg må bare lære litt mer,» tenkte han. «Ingen blir god uten å øve.»": "{name} började öva med bollen, först långsamt och sedan lite snabbare.\nHan försökte dribbla som de stora spelarna gjorde på tv, men bollen rullade inte alltid dit han ville.\n\nHan bet försiktigt ihop tänderna. \"Jag måste bara lära mig lite mer\", tänkte han. \"Ingen blir bra utan att träna.\"",
  "Så kom et skudd som gikk helt feil vei.\n{name} satte seg ned i gresset og kjente skuffelsen prikke bak øynene.\n\nBallen lå stille ved siden av ham, som om den også ventet. «Kanskje jeg ikke er god nok,» tenkte han, selv om han ønsket å være modig.": "Sedan kom ett skott som gick helt åt fel håll.\n{name} satte sig ner i gräset och kände besvikelsen sticka bakom ögonen.\n\nBollen låg stilla bredvid honom, som om den också väntade. \"Kanske är jag inte tillräckligt bra\", tänkte han, även om han ville vara modig.",
  "Treneren kom bort og satte seg rolig ved siden av {name}.\n«Vet du hva de beste spillerne gjør når de bommer?» spurte han. «De prøver én gang til.»\n\n{name} så opp. Treneren smilte varmt. «Jeg tror på deg.» Da kjente {name} motet vende tilbake, lite først, men sterkere for hvert sekund.": "Tränaren kom fram och satte sig lugnt bredvid {name}.\n\"Vet du vad de bästa spelarna gör när de missar?\" frågade han. \"De försöker en gång till.\"\n\n{name} såg upp. Tränaren log varmt. \"Jag tror på dig.\" Då kände {name} modet komma tillbaka, först lite, sedan starkare för varje sekund.",
  "{name} reiste seg, tørket gresset av hendene og prøvde igjen.\nDenne gangen holdt han ballen nær foten og lyttet til rytmen i stegene sine.\n\nHan bommet fortsatt litt, men han stoppet ikke. For hver berøring ble han roligere, og inni seg tenkte han: «Jeg gir meg ikke.»": "{name} reste sig, borstade bort gräset från händerna och försökte igen.\nDen här gången höll han bollen nära foten och lyssnade på rytmen i sina steg.\n\nHan missade fortfarande ibland, men han slutade inte. För varje bolltouch blev han lugnare, och inom sig tänkte han: \"Jag ger inte upp.\"",
  "Etter hvert begynte føttene å forstå det hjertet allerede ville.\n{name} vendte, driblet og fant små åpninger mellom kjeglene på banen.\n\nMålet virket ikke så langt unna lenger. Han smilte for seg selv, for nå merket han det tydelig: øving gjorde ham bedre.": "Efter en stund började fötterna förstå det hjärtat redan ville.\n{name} vände, dribblade och hittade små öppningar mellan konerna på planen.\n\nMålet kändes inte så långt borta längre. Han log för sig själv, för nu märkte han det tydligt: träning gjorde honom bättre.",
  "Senere satt {name} stille i garderoben mens de andre snakket lavt rundt ham.\nDen store kampen nærmet seg, og treneren skulle velge laget.\n\nHjertet hans dunket så fort at han nesten kunne høre det i rommet. Han håpet, men turte nesten ikke håpe for mye.": "Senare satt {name} tyst i omklädningsrummet medan de andra pratade lågt omkring honom.\nDen stora matchen närmade sig, och tränaren skulle välja laget.\n\nHans hjärta dunkade så fort att han nästan kunde höra det i rummet. Han hoppades, men vågade knappt hoppas för mycket.",
  "{name} lyttet nøye da treneren begynte å lese opp navnene.\nEtt navn kom, så et til, og så enda et.\n\nHan knyttet hendene rundt kanten av benken. Da treneren tok en liten pause, holdt {name} pusten og ventet på ordene som kunne forandre hele dagen.": "{name} lyssnade noga när tränaren började läsa upp namnen.\nEtt namn kom, sedan ett till, och sedan ännu ett.\n\nHan knöt händerna runt bänkkanten. När tränaren tog en liten paus höll {name} andan och väntade på orden som kunde förändra hela dagen.",
  "Plutselig hørte han navnet sitt.\n«{name}, du er med i finalen,» sa treneren tydelig.\n\nFørst satt {name} helt stille. Så kjente han smilet vokse, større og større, helt til han nesten ikke klarte å sitte i ro.": "Plötsligt hörde han sitt namn.\n\"{name}, du är med i finalen\", sa tränaren tydligt.\n\nFörst satt {name} helt stilla. Sedan kände han leendet växa, större och större, tills han nästan inte kunde sitta still.",
  "{name} sto i tunnelen med laget rundt seg og kjente hjertet banke hardt.\nForan ham lyste stadion som en egen liten sol i natten.\n\nHan tenkte på alle skuddene, alle bommene og alle gangene han hadde reist seg igjen. Så tok han et skritt frem. Det var hans tur nå.": "{name} stod i tunneln med laget runt sig och kände hjärtat banka hårt.\nFramför honom lyste stadion som en egen liten sol i natten.\n\nHan tänkte på alla skott, alla missar och alla gånger han hade rest sig igen. Sedan tog han ett steg fram. Det var hans tur nu.",
  "{name} gikk ut på den store stadion, og lyden fra tribunen skylte mot ham som bølger.\nLysene skinte så sterkt at gresset glitret under skoene hans.\n\nHan tok et dypt pust og kjente nervene bli til energi. Dette var øyeblikket han hadde drømt om.": "{name} gick ut på den stora stadion, och ljudet från läktaren sköljde mot honom som vågor.\nLjuset sken så starkt att gräset glittrade under hans skor.\n\nHan tog ett djupt andetag och kände nerverna bli till energi. Det här var ögonblicket han hade drömt om.",
  "Kampen startet, og plutselig kom ballen rullende mot {name}.\nHan tok imot den, løftet blikket og så en liten åpning foran seg.\n\nPublikum jublet, men han hørte mest sin egen pust. Beina husket treningen, og {name} driblet framover mot målet.": "Matchen började, och plötsligt kom bollen rullande mot {name}.\nHan tog emot den, lyfte blicken och såg en liten öppning framför sig.\n\nPubliken jublade, men han hörde mest sin egen andning. Benen mindes träningen, och {name} dribblade framåt mot målet.",
  "{name} trakk foten tilbake og skjøt med alt motet han hadde samlet.\nBallen suste gjennom luften, traff nettet og ble liggende der som et lite mirakel.\n\nHele stadion jublet. {name} løftet armene og lo, for nå visste han det: en ekte stjerne gir aldri opp.": "{name} drog foten bakåt och sköt med allt mod han hade samlat.\nBollen susade genom luften, träffade nätet och blev liggande där som ett litet mirakel.\n\nHela stadion jublade. {name} lyfte armarna och skrattade, för nu visste han det: en riktig stjärna ger aldrig upp.",
  "En liten spiller. En stor drøm.\n\nNår {name} får sjansen til å spille sin største kamp noensinne, begynner et spennende eventyr fylt med mot, håp og fotballglede.\n\nFra trening og nervøse øyeblikk i garderoben til den store finalen, får barnet oppleve hvordan det føles å tro på seg selv — og skinne på banen.\n\nEn personlig historie om drømmer, mestring og et mål barnet aldri vil glemme.": "En liten spelare. En stor dröm.\n\nNär {name} får chansen att spela sin största match någonsin börjar ett spännande äventyr fyllt av mod, hopp och fotbollsglädje.\n\nFrån träning och nervösa stunder i omklädningsrummet till den stora finalen får barnet uppleva hur det känns att tro på sig själv och glänsa på planen.\n\nEn personlig berättelse om drömmar, utveckling och ett mål barnet aldrig kommer att glömma."
}
