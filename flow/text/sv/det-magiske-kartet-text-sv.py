...
COVER_TITLE_FONT = os.path.join(SCRIPT_DIR, "PlayfairDisplay.ttf")
...
def _sv_genitive_from_cover_text(text: str) -> str:
    line1 = text.split("\n")[0].strip() if text else ""
    lower = line1.lower()
    if lower.endswith(" och"):
        line1 = line1[:-4].strip()
    return line1 if line1.lower().endswith(("s", "x", "z")) else line1 + "s"
...
def draw_centered_title_cover(img, text):
    ...
    w, h = img.size
    base = min(w, h)
    line1 = _sv_genitive_from_cover_text(text)
    font_small = ImageFont.truetype(COVER_TITLE_FONT, max(50, int(base * (79 / 1024))))
    ...
    def _make_text_mask(size, x, y, txt, fnt):
        mask = Image.new("L", size, 0)
        d = ImageDraw.Draw(mask)
        d.text((x, y), txt, font=fnt, fill=255)
        return mask
    ...
    x1_pos = (w - w1) // 2
    y1_pos = top_y
    y2_pos = y1_pos + h1 - int(h1 * 0.15) + spacing

    gm = _make_text_mask(img.size, x1_pos, y1_pos, line1, font_small)
    gm = gm.filter(ImageFilter.GaussianBlur(max(18, int(font_small.size * 0.35))))
    gm = gm.point(lambda px: int(px * 0.55))
    ...
    logo_path = resolve_front_cover_logo(None)
    if not logo_path:
        print("ADVARSEL: Fant ikke magiska-resa-logo-sv.png")
        return
    ...
    shadow_layer = shadow_layer.filter(ImageFilter.GaussianBlur(max(8, int(target_h * 0.20))))
...
def render_page(page: Dict[str, Any], base_dir: str, out_dir: str) -> List[str]:
    ...
    if page.get("type") == "blank":
        ...
        for block in page.get("blocks", []):
            ...
            if side == "left":
                x1, x2 = int(LEFT_X1 * scale_x), int(LEFT_X2 * scale_x)
            else:
                x1, x2 = int(RIGHT_X1 * scale_x), int(RIGHT_X2 * scale_x)
...
_DREAMPAGE_TRANSLATIONS = {
  ...
  "{name} hadde nesten gjort seg klar for kvelden da han oppdaget noe merkelig.\nMellom bøkene på rommet lå et kart han aldri hadde sett før.\nDet så gammelt og støvete ut, men da han løftet det opp, begynte kantene å gløde svakt i gull.": "{name} hade nästan gjort sig redo för kvällen när han upptäckte något märkligt.\nMellan böckerna på rummet låg en karta han aldrig hade sett förut.\nDen såg gammal och dammig ut, men när han lyfte upp den började kanterna glöda svagt i guld.",
  "Kartet ble varmt i hendene hans.\nSmå tegn av gyllent lys steg opp fra papiret og danset rundt i rommet som hemmelige stjerner.\n{name} holdt pusten.\nDette var ikke et vanlig kart.": "Kartan blev varm i hans händer.\nSmå tecken av gyllene ljus steg upp från papperet och dansade runt i rummet som hemliga stjärnor.\n{name} höll andan.\nDet här var ingen vanlig karta.",
  ...
  "På en av de flyvende øyene hørte {name} en svak lyd.\nBak glødende slyngplanter satt en liten drageunge fast med vingen sin.\nDen så redd ut, men ikke farlig.\n{name} løsnet plantene én etter én, og dragen dyttet snuten takknemlig mot hånden hans.": "På en av de flygande öarna hörde {name} ett svagt ljud.\nBakom glödande slingerväxter satt en liten drakunge fast med sin vinge.\nDen såg rädd ut, men inte farlig.\n{name} lossade växterna en efter en, och draken puffade tacksamt nosen mot hans hand.",
  ...
  "Da {name} gikk gjennom portalen, lukket en glødende luftboble seg rundt ham.\nUtenfor danset vannet stille, og hele undervannsbyen lyste.\nMidt i byen blinket et nytt stjernemerke på kartet.\nDa han rørte ved merket, fløy lyset rett inn i papiret.": "När {name} gick genom portalen slöt sig en glödande luftbubbla runt honom.\nUtanför dansade vattnet tyst, och hela undervattensstaden lyste.\nMitt i staden blinkade ett nytt stjärnmärke på kartan.\nNär han rörde vid märket flög ljuset rakt in i papperet.",
  "Det nye stjernemerket fikk kartet til å lyse kaldt og klart.\nEn portal åpnet seg, og {name} steg ut i en verden av is og nordlys.\nFjellene ruvet rundt ham som blå krystaller.\nDrageungen sto ved siden av ham, klar til å følge ham videre.": "Det nya stjärnmärket fick kartan att lysa kallt och klart.\nEn portal öppnade sig, och {name} steg ut i en värld av is och norrsken.\nFjällen reste sig runt honom som blå kristaller.\nDrakungen stod bredvid honom, redo att följa honom vidare.",
