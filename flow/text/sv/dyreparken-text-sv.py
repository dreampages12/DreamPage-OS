...
DYREPARKEN_DREAMPAGE_FIRST = os.path.join(
    DP_ROOT, "books", "dyreparken", "dreampage-first-dyrepark.png"
)
...
def resolve_front_cover_logo(base_dir: str | None = None) -> str | None:
    ...
    candidates = [
        os.path.join(LOGO_DIR, "djurparken-logo-sv.png"),
        os.path.join(LOGO_DIR, "djurparken-logo.png"),
...
def draw_centered_title_cover(img, text, base_dir: str | None = None):
    ...
    font_small = ImageFont.truetype(COVER_TITLE_FONT, max(50, int(base * (90 / 1024))))
    text_color = (252, 253, 253, 255)
    shadow = (15, 35, 65)
    ...
    lx = ((w - target_w) // 2) + int(5 * (base / 1024))
...
def render_page(page: Dict[str, Any], base_dir: str, out_dir: str) -> List[str]:
    ...
    if page.get("blank_only"):
        ...
        if not os.path.exists(blank_path):
            print("Fant ikke blank-back.png:", blank_path)
            return []
    ...
    if page.get("type") == "blank":
        blank_path = _dp_first(DYREPARKEN_DREAMPAGE_FIRST)
...
_DREAMPAGE_TRANSLATIONS = {
  "{name} i\nDyreparken": "{name} i",
  "Takk for at du kjøpte denne personlige historien!\nI denne boken følger vi {name} på et spennende eventyr i dyreparken.\nHer møter {name} dyr som trenger litt hjelp - og oppdager hvor langt vennlighet kan rekke.\nVi håper historien skaper varme, undring og fine lesestunder.": "Tack för att du köpte den här personliga berättelsen!\nI den här boken följer vi {name} på ett spännande äventyr i djurparken.\nHär möter {name} djur som behöver lite hjälp och upptäcker hur långt vänlighet kan räcka.\nVi hoppas att berättelsen skapar värme, förundran och fina lässtunder.",
  "\"Endelig er dagen her,\" sa {name} og smilte ved porten.\nBak inngangen ventet svingete stier, rare dyrelyder og lukten av varmt gress.\nAlt kjentes nesten magisk, som om parken hadde våknet akkurat for ham.\nSå hvisket vinden mellom trærne: Velkommen til dyreparken. Et eventyr venter.": "\"Äntligen är dagen här\", sa {name} och log vid porten.\nBakom ingången väntade slingrande stigar, märkliga djurläten och doften av varmt gräs.\nAllt kändes nästan magiskt, som om parken hade vaknat just för honom.\nSedan viskade vinden mellan träden: Välkommen till djurparken. Ett äventyr väntar.",
  "{name} gikk videre og fikk øye på en høy giraff.\nDen bøyde den lange halsen ned og smilte vennlig.\n\"Hallo der nede,\" sa giraffen. \"Kan du hjelpe oss i dag?\"\n\"Ja!\" svarte {name} uten å nøle.": "{name} gick vidare och fick syn på en lång giraff.\nDen böjde ner sin långa hals och log vänligt.\n\"Hallå där nere\", sa giraffen. \"Kan du hjälpa oss i dag?\"\n\"Ja!\" svarade {name} utan att tveka.",
  "Giraffen kom helt nær og senket stemmen, som om den delte en viktig hemmelighet.\n\"Elefanten trenger hjelp først,\" sa den. \"Han prøver å nå de beste bladene, men de sitter for høyt.\"\n{name} kjente hjertet hoppe av spenning. \"Da går jeg med en gang,\" sa han, og satte fart langs stien.": "Giraffen kom riktigt nära och sänkte rösten, som om den delade en viktig hemlighet.\n\"Elefanten behöver hjälp först\", sa den. \"Han försöker nå de bästa bladen, men de sitter för högt.\"\n{name} kände hjärtat hoppa av spänning. \"Då går jag direkt\", sa han och skyndade längs stigen.",
  "{name} skyndte seg langs stien mot elefantens område.\nSolen glitret i vannet, og bladene raslet over ham som små grønne flagg.\nHan fulgte de tunge fotsporene i sanden og ropte: \"Jeg kommer, elefant!\"\nNå var eventyret virkelig i gang.": "{name} skyndade längs stigen mot elefantens område.\nSolen glittrade i vattnet, och bladen prasslade över honom som små gröna flaggor.\nHan följde de tunga fotspåren i sanden och ropade: \"Jag kommer, elefant!\"\nNu var äventyret verkligen igång.",
  "Ved gjerdet sto elefanten og løftet snabelen håpefullt mot de grønne greinene.\n{name} samlet friske blader, strakte seg så langt han kunne og rakte dem forsiktig frem.\nElefanten tok imot med snabelen og blunket takknemlig. \"Tusen takk,\" sa den. \"Du reddet dagen min.\"": "Vid staketet stod elefanten och sträckte hoppfullt snabeln mot de gröna grenarna.\n{name} samlade färska blad, sträckte sig så långt han kunde och räckte försiktigt fram dem.\nElefanten tog emot med snabeln och blinkade tacksamt. \"Tusen tack\", sa den. \"Du räddade min dag.\"",
  "Da elefanten var glad igjen, gikk {name} videre med lette steg.\nVed vannet ventet en zebra som stampet urolig med hoven og så nedover stien.\n\"Jeg finner ikke veien tilbake til flokken min,\" sa zebraen. \"Kan du hjelpe meg også?\"\n\"Selvfølgelig,\" sa {name}. \"Vi finner dem sammen.\"": "När elefanten var glad igen gick {name} vidare med lätta steg.\nVid vattnet väntade en zebra som stampade oroligt med hoven och såg längs stigen.\n\"Jag hittar inte tillbaka till min flock\", sa zebran. \"Kan du hjälpa mig också?\"\n\"Självklart\", sa {name}. \"Vi hittar dem tillsammans.\"",
  "Zebraen satte fart, og {name} sprang ved siden av så godt han kunne.\nDe fulgte små spor i sanden, forbi busker og rundt en dam som glitret i solen.\nTil slutt fant zebraen flokken sin igjen. \"Nå går alt bra,\" sa den lykkelig. {name} lo. \"Vi er et godt team.\"": "Zebran satte fart, och {name} sprang bredvid så gott han kunde.\nDe följde små spår i sanden, förbi buskar och runt en damm som glittrade i solen.\nTill slut hittade zebran sin flock igen. \"Nu blir allt bra\", sa den lyckligt. {name} skrattade. \"Vi är ett bra team.\"",
  "Til slutt kom {name} til løven, som satt fast bak en tung gren.\n{name} tok tak og skjøv så godt han kunne.\n\"Du er modigere enn mange voksne,\" sa løven da veien ble fri.\n\"Jeg ville bare hjelpe,\" sa {name}.": "Till slut kom {name} till lejonet, som satt fast bakom en tung gren.\n{name} tog tag och sköt så hårt han kunde.\n\"Du är modigare än många vuxna\", sa lejonet när vägen blev fri.\n\"Jag ville bara hjälpa\", sa {name}.",
  "Da oppdragene var ferdige, kom alle dyrene bort til {name}.\n\"Du hjalp oss alle,\" sa giraffen.\n\"Du er vår venn nå,\" sa elefanten, mens zebraen nikket og løven brummet mykt.\n{name} kjente seg varm av glede.": "När uppdragen var klara kom alla djuren fram till {name}.\n\"Du hjälpte oss alla\", sa giraffen.\n\"Du är vår vän nu\", sa elefanten, medan zebran nickade och lejonet brummade mjukt.\n{name} kände sig varm av glädje.",
  "{name} løftet hånden og vinket til alle de nye vennene sine.\n\"Ha det!\" ropte han, selv om stemmen ble litt myk av savn.\n\"Kom tilbake en dag,\" ropte dyrene tilbake, én etter én.\nDet var trist å gå, men hjertet hans var fullt av lys og varme minner.": "{name} lyfte handen och vinkade till alla sina nya vänner.\n\"Hej då!\" ropade han, även om rösten blev lite mjuk av saknad.\n\"Kom tillbaka en dag\", ropade djuren tillbaka, ett efter ett.\nDet var sorgligt att gå, men hans hjärta var fullt av ljus och varma minnen.",
  "Et stykke borte snudde {name} seg en siste gang.\nI sollyset så det nesten ut som om dyrene smilte til ham fra det fjerne.\nHan la hånden mot brystet og kjente hvor glad han var blitt i dem.\nDette var et farvel som ikke føltes helt som et farvel.": "En bit bort vände sig {name} om en sista gång.\nI solljuset såg det nästan ut som om djuren log mot honom på avstånd.\nHan lade handen mot bröstet och kände hur mycket han hade kommit att tycka om dem.\nDet här var ett farväl som inte riktigt kändes som ett farväl.",
  "Ved stien glitret noe lite i kveldssolen.\n{name} bøyde seg ned og fant en liten elefantfigur i grusen.\n\"Et minne fra oss,\" var det nesten som vinden hvisket.\n\"Da glemmer jeg dere aldri,\" sa {name} stille.": "Vid stigen glittrade något litet i kvällssolen.\n{name} böjde sig ner och fann en liten elefantfigur i gruset.\n\"Ett minne från oss\", var det nästan som vinden viskade.\n\"Då glömmer jag er aldrig\", sa {name} tyst.",
  "Solen sank lavt mens {name} gikk hjemover.\nBak porten lå dyreparken stille, og foran ham ventet huset med varme lys.\nHan holdt elefantfiguren tett og visste det i hele hjertet sitt:\nEventyret var ikke helt over.": "Solen sjönk lågt medan {name} gick hemåt.\nBakom porten låg djurparken stilla, och framför honom väntade huset med varma ljus.\nHan höll elefantfiguren tätt intill sig och visste det i hela sitt hjärta:\nÄventyret var inte riktigt över.",
  "Før {name} sovnet, så han bort på den lille elefanten ved lampen.\nRommet var stille, men han syntes nesten han kunne høre et mykt brøl og et vennlig tramp.\nKanskje, tenkte han, venter dyrevennene på meg igjen i morgen.\nOg med det sovnet han med et smil.": "Innan {name} somnade såg han bort på den lilla elefanten vid lampan.\nRummet var tyst, men han tyckte nästan att han kunde höra ett mjukt rytande och ett vänligt tramp.\nKanske, tänkte han, väntar djurvännerna på mig igen i morgon.\nOch med det somnade han med ett leende.",
  "Denne boken handler om små handlinger som betyr mye.\nNår {name} går inn i dyreparken, venter et varmt og spennende eventyr bak hver sving. En giraff ber om hjelp, en elefant blir lettet, en zebra finner smilet igjen, og en løve oppdager hva ekte vennskap er.\nGjennom møtene lærer {name} at omtanke, mot og vennlighet kan gjøre en stor forskjell.\nEn personlig historie fylt med dyreglede, nærhet og minner som varer lenge etter siste side.": "Den här boken handlar om små handlingar som betyder mycket.\nNär {name} går in i djurparken väntar ett varmt och spännande äventyr bakom varje sväng. En giraff ber om hjälp, en elefant blir lättad, en zebra hittar sitt leende igen och ett lejon upptäcker vad riktig vänskap är.\nGenom mötena lär sig {name} att omtanke, mod och vänlighet kan göra stor skillnad.\nEn personlig berättelse fylld av djurglädje, närhet och minnen som varar länge efter sista sidan."
}
