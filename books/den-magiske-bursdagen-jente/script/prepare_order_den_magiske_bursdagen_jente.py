...
# === KONFIGURASJON ==========================
BOOK_ROOT = Path(r"C:\DreamPage-OS\books\den-magiske-bursdagen-jente")
BASE_DIR = BOOK_ROOT / "base"
ORDERS_DIR = BOOK_ROOT / "orders"
COMFY_OUTPUT_ROOT = Path(r"C:\DreamPage-OS\output") / "den-magiske-bursdagen-jente" / "orders"
...
PAGE_TO_BASE_STEM = {
    "page00": "forside(magiske-bursdag-jente)",
}
for _n in range(1, 15):
    PAGE_TO_BASE_STEM[f"page{_n:02d}"] = f"{_n:02d}(magiske-bursdag-jente)"
...
def main(order_id: str) -> None:
    ...
    # 1) Kopier alle base-bilder forst
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Bruk: python prepare_order_den_magiske_bursdagen_jente.py <order_id>")
        raise SystemExit(1)
